...
def calcul_temps_KMC(x, y, nb_pas_temps, iterations):
    ...
    elapsed_time = []
    kT, param_mu = 1.0, 0.25

    for i in range(iterations):
        start_time = time.time()
        KMC2D(grid, kT, param_mu, nb_pas_temps, gif=False, gamma=False, rugosity=False)
        end_time = time.time()
...
def calcul_temps_allonger(x, y, nb_pas_temps_max, iterations):
    ...
    nb_pas = np.arange(1,nb_pas_temps_max,10)
    mean_time = []
    std_time = []
    ...
    def linear_model(x, a, b):  
        return a * np.exp(x + b)
    popt, pcov = curve_fit(linear_model, nb_pas, mean_time, sigma=std_time, absolute_sigma=True)
    a_fit, b_fit = popt
    ...
    plt.plot(nb_pas, linear_model(nb_pas, *popt), 'r-', linewidth=1, label=f'Régression linéaire')
    plt.xlabel('Nombre de pas de temps', fontsize=16)
    plt.ylabel('Temps moyen (s)', fontsize=16)
    plt.title(f'Temps de calcul pour KMC \npour un substrat {x}x{y} (itération {iterations})', fontsize=16)
    plt.grid()
    plt.show()

# calcul_temps_allonger(x=50, y=100, nb_pas_temps_max=100, iterations=5)
