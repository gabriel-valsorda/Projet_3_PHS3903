...
def KMC2D_aveccoms(config,positions_surface, kT, deltamu, nb_pas_temps, gif=False, gamma=False, rugosity=False):
    ...
    for iteration in range(nb_pas_temps):
        ...
        if evnt[1]==2: #ALORS ON A UNE DIFFUSION! ON TROUVE DONC LE PLUS PROCHE SITE COMPATIBLE
            ...
            print(f"Le site du changement est {site_changement}")
            print(f"L'atome est {atome}")
            parite=((positions_surface[gauche][1]-1)+gauche)%2
            if parite==0:
                sitegauche='Na'
            else :
                sitegauche='Cl'
            print(f"Le site gauche est {sitegauche}, à la position {gauche} ")
            while sitegauche==atome:
                ...
                if parite==0:
                    ...
                    sitegauche='Cl'
                print(f"Le site gauche est {sitegauche}, à la position {gauche} ")
            print(f"La valeur finale de gauche est {gauche}")
            proximite_gauche=site_changement-gauche

            print(f"La valeur de proximité gauche est {proximite_gauche}")

            if proximite_droite<proximite_gauche:
                #ON A ALORS DIFFUSION À DROITE
                print(f"{iteration}  Diffusion à droite au site {site_changement} type 1")
            ...
            if proximite_gauche<proximite_droite:
                ...
                print(f"{iteration}  Diffusion à gauche au site {site_changement} type 2")
                config[site_changement][positions_surface[site_changement][1]-1]=None
                if atome=='Na':
            ...
            if proximite_gauche==proximite_droite:
                ...
                if rprimeprime<=0.5:
                    #diffusion à droite
                    print(f"{iteration}  Diffusion à droite au site {site_changement} type 3")
                    config[site_changement][positions_surface[site_changement][1]-1]=None
                    if atome=='Na':
                        config[droite][positions_surface[droite][1]]=1
                    else : 
                ...
                if rprimeprime>0.5:
                    #diffusion à gauche
                    print(f"{iteration}  Diffusion à gauche au site {site_changement} type 4")
                    config[site_changement][positions_surface[site_changement][1]-1]=None
                    if atome=='Na':
                        print('Na')
                        config[gauche][positions_surface[gauche][1]]=1
